...
async function setupRoleManagementViaAPI() {
  console.log('🚀 CONFIGURATION VIA API REST SUPABASE\n');
  ...
  try {
    ...
    // 2. Créer les tables via des requêtes HTTP directes
    console.log('\n2️⃣ Création des tables via API...');
...
async function createTablesViaAPI() {
  // Note: L'API REST de Supabase ne permet pas de créer des tables directement
  // Il faut utiliser l'interface web ou l'API SQL
  console.log('⚠️  L\'API REST ne permet pas de créer des tables directement');
  console.log('📋 Les tables doivent être créées manuellement dans Supabase');
  console.log('🔗 Allez sur: https://supabase.com/dashboard');
  console.log('📝 SQL Editor → New Query → Collez le script SQL');
  
  // Afficher le script SQL à exécuter
  console.log('\n📄 SCRIPT SQL À EXÉCUTER:');
  console.log('='.repeat(60));
  
  const sqlScript = `-- Script pour créer les tables de gestion des rôles et permissions granulaires
...
async function insertInitialData() {
  console.log('⚠️  Les données seront insérées après la création des tables');
  console.log('📋 Exécutez d\'abord le script SQL ci-dessus');
}

async function verifySetup() {
  console.log('⚠️  Vérification impossible sans les tables');
  console.log('📋 Créez d\'abord les tables avec le script SQL');
}

// Exécution du script
setupRoleManagementViaAPI();
